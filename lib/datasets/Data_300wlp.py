...
class WLP(data.Dataset):
    def __init__(self, split = 'train', imSize=(224,224),aug=True):

        self.imSize = imSize
        self.wlp_root='./data/300W_LP'

        print('Loading 300WLP dataset...')
        ...
        self.trainidx=['AFW','AFW_Flip','HELEN','HELEN_Flip','IBUG','IBUG_Flip','LFPW','LFPW_Flip']
        #self.testidx=['AFLW2000']
        self.indices=self._get_db()
        
        self.aug=aug
            
        print('Loaded 300WLP dataset split "%s" with %d records...' % (split, len(self.indices)))
    ...
    def _get_db(self):
        ...
        gt_db = self._load_samples(self.trainidx,self.wlp_root)
    ...
    def _load_samples(self,idx_list,root_path):
        ...
        for idx in idx_list:
            ...
            for i in range(e_idx):
                ...
                pose_para,pt2d_x,pt2d_y=self._read_mat_file(os.path.join(matfile_path,mat_files[i]))
                
                pitch = pose_para[0] * 180 / np.pi
                yaw = pose_para[1] * 180 / np.pi
                roll = pose_para[2] * 180 / np.pi
                
                if max(abs(pitch),abs(yaw),abs(roll))>99:
                   continue
    ...
    def _read_mat_file(self,matfile):
        ...
        return pose_para,pt2d_x,pt2d_y
